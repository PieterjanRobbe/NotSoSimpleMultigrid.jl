...
function coarsen(A::SparseMatrixCSC, sz::NTuple, R_op::TransferKind, P_op::TransferKind, ngrids::NTuple)
    ...
    d = length(ngrids)
    grids = Array{Grid{typeof(A),Vector{eltype(A)},NTuple{d,typeof(A)},typeof(sz)},length(sz)}(undef, ngrids...)
    grids[1] = Grid(A,zero_x(A),zero_x(A),tuple(R̃.(1:d,R_op,sz...)...),ntuple(i->spzeros(0,0),d),sz)
    for (idx,i) in enumerate(Base.Iterators.drop(Base.product(UnitRange.(1,ngrids)...),1))
        sz_c = sz.>>(i.-1)
...
function coarsen(f::Function, sz::NTuple, R_op::TransferKind, P_op::TransferKind, ngrids::NTuple)
    ...
    d = length(ngrids)
    A0 = f(sz...) 
    grids = Array{Grid{typeof(A0),Vector{eltype(A0)},NTuple{d,typeof(A0)},typeof(sz)},length(sz)}(undef, ngrids...)
...
norm_of_residu(grid::Grid) = SimpleMultigrid.norm_of_residu(grid)
